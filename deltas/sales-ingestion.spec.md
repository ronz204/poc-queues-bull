...
- Continuously generating and inserting new rows into `sales.transactions`, referencing existing products/regions.
...
- Does not compute or read report aggregates — this slice only ever writes to `sales.transactions`.
...
- Each tick inserts a random count of new `transactions` rows, between 1 and 5.
...
2. Every inserted transaction references a `product_id`/`region_id` that actually exists at insertion time — enforced by the foreign-key constraints on `transactions`, and the generator itself must only ever pick from currently-seeded rows, never invent an id.
...
5. `occurredAt` is not required to equal the row's insertion time — consistent with, not a new constraint beyond, the existing `occurred_at`/`created_at` split already in place for `transactions`.
...
- `transactions` grows continuously while the worker process runs — a manual or automated check confirms the row count increases over a several-tick window with no manual trigger.
